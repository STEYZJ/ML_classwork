...
from src.neural_network_lab import run


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run lab 4: neural-network classifiers.")
    parser.add_argument(
        "--experiment-root",
        type=Path,
        default=Path("../../../../实验"),
...
def main() -> None:
    ...
    rows = run(args.experiment_root, out)
    save_rows(rows, out / "neural_network_metrics.csv")
    (out / "summary.json").write_text(
        json.dumps({"neural_network": rows}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
